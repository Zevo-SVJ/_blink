...
import { computeBlinkScore, getTier, tierLabel, scoreOutOfTen } from "@/lib/ranking";
        ...
        <>
          ...
          <Group title={t.analysis.standing}>
            <div className="flex items-stretch">
              {/* Out of ten, like the verdict at the top of this same screen.
                  It said 590 while the score above it said 6.0, both labelled
                  "Blink Score" — one number, two units, on one page. */}
              <Fact
                label={t.analysis.blinkScore}
                value={scoreOutOfTen(score)}
                sub={tierLabel(tier, t)}
                accent
              />
              <Fact
  ...
  return (
    <Group title={t.analysis.topMoves}>
      <p className="text-xs leading-relaxed text-white/50">
        You&rsquo;re at{" "}
        <span className="t-numeric font-bold text-white/80">{scoreOutOfTen(score)}</span> —{" "}
        {tierLabel(tier, t)}.
